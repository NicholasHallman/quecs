...
if(isMainThread) {
    for (let i = 0; i < 5; i++){
        ...
        const worker = new Worker("./dist/threaded.js");
        workerPool.push(worker);
    }
} else {
    // thread handler
    parentPort.on('message', ({func, eids, comps, flag}) => {
        eval(func)(eids, comps);
        flag[0] = 1;
    });
}
...
export const threadedSystem = (queryComp: Component[], func: System) => world => {
    resetWorkerStatus();
    
    const query = world.defineQuery(...queryComp);
    const eids = [...query()];
